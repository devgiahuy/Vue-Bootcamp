...
// defineEmits dùng để định nghĩa các event mà component sẽ emit ra ngoài.
// Khi component emit event, parent component có thể lắng nghe event đó và thực hiện hành động tương ứng.
// Ví dụ: khi component emit event 'update:modelValue', parent component có thể lắng nghe event đó và thực hiện hành động tương ứng.
// Đây là cách khai báo emits trong Composition API.
// Trong Options API, ta dùng `emits: ['update:modelValue']`.
// Event này sẽ được emit ra ngoài khi component emit event 'update:modelValue'.
// Parent component có thể lắng nghe event này bằng cách dùng `@update:modelValue="handleUpdateModelValue"`.
